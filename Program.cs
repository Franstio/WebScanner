...
{
    _log = _log.Enrich.FromLogContext().Filter.ByIncludingOnly( x=>x.Level == Serilog.Events.LogEventLevel.Warning && Matching.FromSource<ArduinoService>()(x)).WriteTo.File(Path.Combine(basepath, "timbangan-reset.txt")).Enrich.FromLogContext().MinimumLevel.Error().WriteTo.Console()
